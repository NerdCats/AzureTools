...
$storageAccount = (Get-AzureStorageAccount).Label ` | Out-GridView ` -Title "Select Azure Storage Account" ` -PassThru
...
$disk = $vm ` | Get-AzureDataDisk ` | Out-GridView ` -Title "Select a data disk to resize" ` -PassThru
...
do {
    $size = Read-Host ` -Prompt "New size in GB"
}
...
"Please go to your azure portal and extend the disk size using Disk Management"